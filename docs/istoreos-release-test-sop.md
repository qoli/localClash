...
每個發版案例都必須指向候選實際提供的正式 UI／RPC 入口與現行功能契約。
尚未落地的產品構想應留在設計文件，不得先寫成現行發版 gate；入口或契約已移除
時應校正本 SOP，而不是把不存在的操作標成產品 BLOCKED 或要求使用者逐次豁免。
...
4. 核心專屬改動驗該核心；跨核心共用 runtime／渲染契約則驗 Meta、Smart 的
   受影響路徑。涉及核心選擇或專屬配置時，使用各核心正式初始化入口；涉及模型
   才加入相關模型斷言。
   「驗兩個核心的重啟」不等於「雙核心 A–E、K1–K7 全部重跑」。
...
結果、恢復方式。core_scope 為 `meta`、`smart` 或本文件允許的 `shared`；
不能以一筆「Mihomo 通過」代表全部。
...
- 完整發版驗收中修正程式或替換候選資產，產生新的候選識別並重跑受影響的情境、V、
  ...
  K、V/N；成對更新邏輯改變時 K7 必須在兩種活躍核心下重跑。
...
在 Meta、Smart 獨立基線執行**，包括 C 的舊版升級初始化及 D 的已配置升級，
不再允許單核心代測。
...
| K1–K7 | K1–K3、K5–K7 各自 Meta、Smart；K4 僅 Smart | K4 不建立 Meta 模型案例，不以 N/A 免除 Smart 必測項；兩核心由獨立正式初始化基線驗證 |
...
已驗證，反之亦然。K7 必須在兩種活躍核心各跑；另一個新檔的可運行性由另一核心
...
分別執行 B/D/E；策略同步開啟仍是主線，不因自訂策略會被覆蓋就關掉它。
...
N1–N6 分別綁定 Meta／Smart 與冷暖狀態；兩核心各自重跑 N1–N4，不能借用
另一核心的流量結果。以下均為 `required_function`，N5 的
...
cases:
  - id: <A-meta-or-K7-smart>
    attempt_id: <unique-attempt-id-never-overwritten>
    core_scope: <meta-smart-or-shared>
